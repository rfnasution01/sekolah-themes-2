...
export function MenubarColor({ color }: { color: string }) {
  ...
  return (
    <Menubar className="px-4">
      <MenubarMenu>
        <MenubarTrigger
          className="w-full text-center text-white opacity-20 transition-all duration-300 hover:cursor-pointer hover:opacity-90"
          variant="nothing"
          layout="icon"
          size="fit"
          onClick={handleMenuClick}
        >
          <span className={`rounded-2xl ${bgPrimary500(color)} p-12`}>
            <Settings size={24} />
          </span>
        </MenubarTrigger>
        {isMenuOpen && (
